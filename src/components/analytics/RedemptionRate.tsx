...
const RedemptionRate = () => {
  const [vouchers, setVouchers] = useState<VoucherWithRate[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchVoucherRedemptionRates = async () => {
      try {
        const response = await api.get('/api/business-analytics/voucher-redemption-rate');
        if (response.status === 200) {
          setVouchers(response.data.vouchers);
        }
      } catch (err) {
        console.error('Error fetching redemption rates:', err);
      } finally {
        setLoading(false);
      }
    };

    fetchVoucherRedemptionRates();
  }, []);
