...
const ProfilePage: React.FC = () => {
  ...
  const [isOutletModalVisible, setIsOutletModalVisible] =
    useState<boolean>(false); // Modal for outlet deletion
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      api
        .post(`/api/business/profile/uploadImage`, formData, {
          headers: {
            "Content-Type": "multipart/form-data", // for file uploads
          },
        })
  ...
  const handleConfirmDeleteOutlet = async (contact: string) => {
    ...
    try {
      ...
      const response = await api.delete(
        `/api/business/outlets/${selectedOutlet.outlet_id}`
      );
      if (response.status === 200) {
        setOutlets(
          outlets.filter(
        ...
        ); // create a new outlets array and exclude the outlet with the given id
        setError(null);
      } else {
          ...
          <img
            src={
              ...
              (profileImage
                ...
                : "/images/profile.png")
            } // defualt image if failed to load
            alt="Profile"
            width="96"
            height="96"
