...
const OutletBusinessSubscription = () => {
  ...
  const total_gem =
    pricing.base[duration].gems + (pricing.extra[distance_coverage]?.gems || 0);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!outlet) {
        setError("No outlet found in the query parameters");
        return;
      }
      ...
      const response = await api.post(
        `/api/business/outlet/subscription/${username}/${outlet}`,
        payload
      );
...
export default withAuth(OutletBusinessSubscription);
